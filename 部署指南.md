# 🚀 K8s 登录系统 - 完整部署指南

> 一个基于 Kubernetes 的前后端分离登录系统  
> 前端：Vue3 + Vite + Nginx | 后端：Django + DRF | 数据库：PostgreSQL

---

## 📋 目录

1. [系统架构](#系统架构)
2. [快速开始（5分钟）](#快速开始)
3. [详细部署步骤](#详细部署步骤)
4. [访问配置](#访问配置)
5. [常见问题](#常见问题)
6. [管理维护](#管理维护)

---

## 🏗️ 系统架构

```
用户浏览器
    ↓
┌─────────────────────────────────────────┐
│   Kubernetes 集群 (Minikube)            │
│                                         │
│  ┌──────────────┐    ┌──────────────┐  │
│  │  Frontend    │───→│  Backend     │  │
│  │  Vue3+Nginx  │    │  Django+DRF  │  │
│  │  (2 pods)    │    │  (3 pods)    │  │
│  └──────────────┘    └──────┬───────┘  │
│                              │          │
│                      ┌───────▼───────┐  │
│                      │  PostgreSQL   │  │
│                      │  (1 pod+PV)   │  │
│                      └───────────────┘  │
└─────────────────────────────────────────┘
```

### 核心特性

- ✅ 前后端分离，独立扩展
- ✅ 高可用部署（多副本）
- ✅ 数据持久化（PostgreSQL PV）
- ✅ JWT Token 认证
- ✅ 自动健康检查和恢复
- ✅ 域名访问支持

---

## 🚀 快速开始

### 前置条件

确保已安装：
- ✅ Docker >= 20.10
- ✅ Kubernetes (Minikube)
- ✅ kubectl >= 1.19

### 一键部署

```bash
# 1. 进入项目目录
cd /root/learn/01-k8s

# 2. 配置 Docker 环境（使用 Minikube 的 Docker）
eval $(minikube docker-env)

# 3. 构建镜像（约2-3分钟）
docker build -t login-backend:latest ./backend
docker build -t login-frontend:latest ./frontend

# 4. 创建持久化存储目录
minikube ssh "sudo mkdir -p /mnt/data/postgres && sudo chmod 777 /mnt/data/postgres"

# 5. 一键部署（约1-2分钟）
./scripts/deploy.sh

# 6. 等待部署完成
kubectl get pods -n login-system -w
```

**完成！** 🎉 等待所有 Pod 状态变为 `Running` 即可访问。

---

## 📝 详细部署步骤

### 步骤 1：准备 Kubernetes 环境

#### 检查 Minikube 状态

```bash
minikube status
```

如果未运行，执行：

```bash
# 使用国内镜像加速安装脚本
./scripts/install-k8s-cn.sh
```

#### 验证集群

```bash
kubectl cluster-info
kubectl get nodes
```

---

### 步骤 2：构建 Docker 镜像

#### 配置 Docker 环境

```bash
# 重要：使用 Minikube 的 Docker 守护进程
eval $(minikube docker-env)

# 验证
docker info | grep -i name
# 应该显示：Name: minikube
```

#### 构建镜像

```bash
# 构建后端镜像（已配置阿里云镜像源）
cd /root/learn/01-k8s/backend
docker build -t login-backend:latest .

# 构建前端镜像（已配置淘宝 npm 镜像源）
cd /root/learn/01-k8s/frontend
docker build -t login-frontend:latest .
```

#### 验证镜像

```bash
eval $(minikube docker-env)
docker images | grep login
```

应该看到：
```
login-backend    latest    116MB
login-frontend   latest    53MB
```

---

### 步骤 3：创建持久化存储

```bash
# 在 Minikube 节点中创建目录
minikube ssh "sudo mkdir -p /mnt/data/postgres && sudo chmod 777 /mnt/data/postgres"

# 验证
minikube ssh "ls -ld /mnt/data/postgres"
```

---

### 步骤 4：部署到 Kubernetes

#### 使用自动部署脚本（推荐）

```bash
cd /root/learn/01-k8s
./scripts/deploy.sh
```

脚本会自动：
1. 创建命名空间 `login-system`
2. 部署 PostgreSQL（Secret、ConfigMap、PV、PVC、Deployment、Service）
3. 部署 Backend（Secret、ConfigMap、Deployment、Service）
4. 部署 Frontend（ConfigMap、Deployment、Service）
5. （可选）部署 Ingress
6. 显示访问信息

#### 手动逐步部署

```bash
cd /root/learn/01-k8s

# 1. 创建命名空间
kubectl apply -f k8s/namespace.yaml

# 2. 部署 PostgreSQL
kubectl apply -f k8s/postgres/
kubectl wait --for=condition=ready pod -l app=postgres -n login-system --timeout=120s

# 3. 部署后端
kubectl apply -f k8s/backend/
kubectl wait --for=condition=ready pod -l app=backend -n login-system --timeout=120s

# 4. 部署前端
kubectl apply -f k8s/frontend/
kubectl wait --for=condition=ready pod -l app=frontend -n login-system --timeout=120s

# 5. (可选) 部署 Ingress
kubectl apply -f k8s/ingress.yaml
```

---

### 步骤 5：验证部署

```bash
# 查看所有资源
kubectl get all -n login-system

# 查看 Pod 状态
kubectl get pods -n login-system
```

✅ 所有 Pod 的 STATUS 都应该是 `Running`，READY 应该是 `1/1`。

---

## 🌐 访问配置

### 方案对比

| 方案 | 适用场景 | 简单度 | 稳定性 | 域名支持 |
|------|---------|--------|--------|---------|
| **Port Forward (8080)** | 快速测试 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ❌ |
| **NodePort** | 稳定使用 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ❌ |
| **Ingress (WSL2内部)** | WSL2内域名访问 | ⭐⭐⭐ | ⭐⭐⭐⭐ | ✅ |
| **Port Forward + Windows hosts** | Windows域名访问 | ⭐⭐⭐ | ⭐⭐⭐ | ✅ |

---

### 方案 1：Port Forward（最简单）⭐推荐

适用于快速测试和开发。

#### WSL2 中访问

```bash
# 转发到本地 8080 端口
kubectl port-forward -n login-system service/frontend-service 8080:80
```

浏览器访问：**http://localhost:8080**

#### Windows 中访问

```bash
# 在 WSL2 中执行（监听所有网卡）
kubectl port-forward -n login-system service/frontend-service 8080:80 --address 0.0.0.0 &

# 获取 WSL2 IP
WSL_IP=$(hostname -I | awk '{print $1}')
echo "Windows 访问: http://$WSL_IP:8080"
```

在 Windows 浏览器中访问：**http://172.19.166.119:8080**

**或使用自动配置脚本：**

```bash
cd /root/learn/01-k8s
./scripts/setup-windows-access.sh
# 选择方式 2
```

---

### 方案 2：NodePort（最稳定）⭐推荐

适用于稳定使用，无需额外配置。

#### 查看 NodePort

```bash
kubectl get svc frontend-service -n login-system
```

输出示例：
```
NAME               TYPE       CLUSTER-IP     PORT(S)        AGE
frontend-service   NodePort   10.107.45.36   80:30080/TCP   1h
```

#### 访问地址

```bash
# 使用 Minikube IP（推荐）
MINIKUBE_IP=$(minikube ip)
echo "访问: http://$MINIKUBE_IP:30080"
# http://192.168.49.2:30080

# 或使用 WSL2 IP
WSL_IP=$(hostname -I | awk '{print $1}')
echo "访问: http://$WSL_IP:30080"
# http://172.19.166.119:30080
```

**Windows 浏览器访问**：
- `http://192.168.49.2:30080`
- `http://172.19.166.119:30080`

---

### 方案 3：Ingress + 域名（WSL2 内部）

适用于 WSL2 内部使用域名访问。

#### 启用 Ingress

```bash
# 1. 检查 Ingress Controller
kubectl get pods -n ingress-nginx

# 2. 部署 Ingress
kubectl apply -f k8s/ingress.yaml

# 3. 启动 Minikube Tunnel
nohup minikube tunnel > /tmp/minikube-tunnel.log 2>&1 &

# 4. 配置 hosts
sudo bash -c 'echo "127.0.0.1  login.local www.login.local api.login.local" >> /etc/hosts'

# 5. 测试访问
curl http://login.local/api/health/
```

#### 访问地址

- 前端主站：`http://login.local`
- 注册页面：`http://login.local/register`
- 登录页面：`http://login.local/login`
- API 接口：`http://api.login.local/api/health/`

---

### 方案 4：Port Forward + Windows hosts

适用于 Windows 中使用域名访问。

#### WSL2 配置

```bash
# 1. 启动 port-forward（监听所有网卡）
kubectl port-forward -n login-system service/frontend-service 8080:80 --address 0.0.0.0 &

# 2. 获取 WSL2 IP
hostname -I | awk '{print $1}'
# 例如：172.19.166.119
```

#### Windows 配置

**步骤 1：修改 Windows hosts 文件**

1. 以管理员身份运行记事本
2. 打开文件：`C:\Windows\System32\drivers\etc\hosts`
3. 在文件末尾添加：

```
# K8s Login System (WSL2)
172.19.166.119  login.local www.login.local api.login.local
```

4. 保存文件

**步骤 2：清除 DNS 缓存**

在 PowerShell（管理员）中执行：

```powershell
ipconfig /flushdns
```

**步骤 3：访问**

Windows 浏览器中打开：**http://login.local:8080**

---

### 快速配置脚本

使用自动化脚本简化配置：

```bash
cd /root/learn/01-k8s
./scripts/setup-windows-access.sh
```

选项：
1. **NodePort 方式**（最稳定）- 无需额外配置
2. **Port-forward 到 8080**（最简单）- 适合快速测试
3. **Port-forward + 域名**（高级）- 需要修改 Windows hosts

---

## 🧪 功能测试

### 1. 健康检查

```bash
# 后端 API
curl http://localhost:8080/api/health/
# 预期：{"status":"healthy","service":"authentication-api","version":"1.0.0"}
```

### 2. 用户注册

**浏览器测试：**
1. 访问 http://localhost:8080/register
2. 填写注册信息
3. 点击"注册"按钮
4. ✅ 应显示"注册成功！3秒后跳转到登录页面..."

**命令行测试：**

```bash
curl -X POST http://localhost:8080/api/register/ \
  -H "Content-Type: application/json" \
  -d '{
    "username": "testuser",
    "email": "test@example.com",
    "password": "test123456",
    "password2": "test123456"
  }'
```

### 3. 用户登录

**浏览器测试：**
1. 访问 http://localhost:8080/login
2. 输入用户名和密码
3. 点击"登录"
4. ✅ 应跳转到首页并显示用户信息

**命令行测试：**

```bash
curl -X POST http://localhost:8080/api/token/ \
  -H "Content-Type: application/json" \
  -d '{
    "username": "testuser",
    "password": "test123456"
  }'
```

### 4. 验证数据库

```bash
# 进入 PostgreSQL 容器
kubectl exec -it -n login-system deployment/postgres-deployment -- psql -U postgres -d logindb

# 查看用户表
\dt
SELECT * FROM users;

# 退出
\q
```

---

## ❓ 常见问题

### Q1: Pod 一直处于 Pending 状态

**原因：** PV 无法绑定或资源不足

**解决：**

```bash
# 检查 PV/PVC 状态
kubectl get pv,pvc -n login-system

# 确保存储目录存在
minikube ssh "sudo ls -ld /mnt/data/postgres"

# 查看 Pod 详细信息
kubectl describe pod <pod-name> -n login-system
```

### Q2: 后端 Pod 启动失败

**原因：** 数据库未就绪或连接失败

**解决：**

```bash
# 查看后端日志
kubectl logs -n login-system deployment/backend-deployment

# 检查数据库是否运行
kubectl get pods -n login-system -l app=postgres

# 测试数据库连接
kubectl exec -it -n login-system deployment/backend-deployment -- sh
nc -zv postgres-service 5432
```

### Q3: 镜像拉取失败 (ImagePullBackOff)

**原因：** 镜像不在 Minikube 的 Docker 中

**解决：**

```bash
# 确保使用 Minikube 的 Docker
eval $(minikube docker-env)

# 重新构建镜像
docker build -t login-backend:latest ./backend
docker build -t login-frontend:latest ./frontend

# 验证镜像存在
docker images | grep login

# 重启 Pod
kubectl rollout restart deployment/backend-deployment -n login-system
kubectl rollout restart deployment/frontend-deployment -n login-system
```

### Q4: Windows 无法访问

**原因：** WSL2 网络隔离或防火墙阻止

**解决：**

```bash
# 1. 检查 port-forward 是否运行
ps aux | grep port-forward

# 2. 重新启动 port-forward（监听所有网卡）
kubectl port-forward -n login-system service/frontend-service 8080:80 --address 0.0.0.0 &

# 3. 获取 WSL2 IP
hostname -I

# 4. Windows 中测试
# PowerShell: Test-NetConnection 172.19.166.119 -Port 8080
```

**Windows 防火墙设置：**

```powershell
# PowerShell（管理员）
New-NetFirewallRule -DisplayName "WSL2 K8s" -Direction Inbound -LocalPort 8080,30080 -Protocol TCP -Action Allow
```

### Q5: 域名无法访问

**WSL2 内部（Ingress）：**

```bash
# 1. 检查 tunnel 是否运行
ps aux | grep "minikube tunnel"

# 2. 重启 tunnel
pkill -f "minikube tunnel"
nohup minikube tunnel > /tmp/minikube-tunnel.log 2>&1 &

# 3. 检查 hosts 文件
cat /etc/hosts | grep login.local

# 4. 测试访问
curl http://login.local/api/health/
```

**Windows（Port Forward + hosts）：**

```powershell
# 1. 检查 hosts 文件
type C:\Windows\System32\drivers\etc\hosts | findstr login.local

# 2. 清除 DNS 缓存
ipconfig /flushdns

# 3. 测试域名解析
ping login.local

# 4. 测试访问
curl http://login.local:8080/api/health/
```

### Q6: WSL2 IP 变化导致无法访问

**原因：** WSL2 重启后 IP 可能变化

**解决：**

```bash
# 1. 获取新的 WSL2 IP
hostname -I | awk '{print $1}'

# 2. 更新 Windows hosts 文件
# 将 172.19.166.119 替换为新的 IP

# 3. 清除 Windows DNS 缓存
# PowerShell: ipconfig /flushdns
```

**推荐：使用 Minikube IP（更稳定）**

```bash
minikube ip
# 192.168.49.2
```

---

## 🛠️ 管理维护

### 查看状态

```bash
# 查看所有资源
kubectl get all -n login-system

# 查看 Pod 详细信息
kubectl describe pod <pod-name> -n login-system

# 查看日志
kubectl logs -f deployment/backend-deployment -n login-system
kubectl logs -f deployment/frontend-deployment -n login-system
kubectl logs -f deployment/postgres-deployment -n login-system

# 查看事件
kubectl get events -n login-system --sort-by='.lastTimestamp'
```

### 扩缩容

```bash
# 扩展后端到 5 个副本
kubectl scale deployment backend-deployment --replicas=5 -n login-system

# 扩展前端到 3 个副本
kubectl scale deployment frontend-deployment --replicas=3 -n login-system

# 查看扩容状态
kubectl get pods -n login-system -w
```

### 更新部署

```bash
# 重新构建镜像
eval $(minikube docker-env)
docker build -t login-backend:v2 ./backend

# 更新镜像（滚动更新）
kubectl set image deployment/backend-deployment \
  backend=login-backend:v2 -n login-system

# 查看滚动更新状态
kubectl rollout status deployment/backend-deployment -n login-system

# 回滚
kubectl rollout undo deployment/backend-deployment -n login-system
```

### 重启服务

```bash
# 重启后端
kubectl rollout restart deployment/backend-deployment -n login-system

# 重启前端
kubectl rollout restart deployment/frontend-deployment -n login-system

# 重启数据库（谨慎操作）
kubectl rollout restart deployment/postgres-deployment -n login-system
```

### 进入容器

```bash
# 进入后端容器
kubectl exec -it -n login-system deployment/backend-deployment -- sh

# 进入前端容器
kubectl exec -it -n login-system deployment/frontend-deployment -- sh

# 进入数据库容器
kubectl exec -it -n login-system deployment/postgres-deployment -- sh
```

### 数据备份与恢复

#### 手动备份

```bash
# 使用备份脚本（推荐）
cd /root/learn/01-k8s
./scripts/backup.sh

# 手动备份
kubectl exec -n login-system deployment/postgres-deployment -- \
  pg_dump -U postgres logindb > backup_$(date +%Y%m%d).sql
```

#### 恢复备份

```bash
# 使用恢复脚本（推荐）
./scripts/restore.sh backups/logindb_backup_20251127_120000.sql.gz

# 手动恢复
cat backup_20251127.sql | kubectl exec -i -n login-system deployment/postgres-deployment -- \
  psql -U postgres -d logindb
```

#### 自动备份（定时任务）

```bash
# 编辑 crontab
crontab -e

# 添加每天凌晨2点自动备份
0 2 * * * cd /root/learn/01-k8s && ./scripts/backup.sh >> /var/log/k8s-backup.log 2>&1

# 查看备份日志
tail -f /var/log/k8s-backup.log
```

#### 备份到远程服务器

```bash
# 备份后上传到远程服务器
./scripts/backup.sh && \
  scp backups/logindb_backup_$(date +%Y%m%d)*.sql.gz user@remote-server:/backups/

# 或使用 rsync
./scripts/backup.sh && \
  rsync -avz backups/ user@remote-server:/backups/k8s-login-system/
```

### 清理资源

```bash
# 方法 1：使用清理脚本
cd /root/learn/01-k8s
./scripts/cleanup.sh

# 方法 2：手动删除
kubectl delete namespace login-system
kubectl delete pv postgres-pv

# 清理 Minikube 中的数据
minikube ssh "sudo rm -rf /mnt/data/postgres"

# 清理 hosts 文件（如果配置了域名）
sudo sed -i '/# K8s Login System/,+1d' /etc/hosts
```

---

## 📊 资源配置说明

### 部署资源清单

| 资源类型 | 名称 | 副本数 | 内存请求 | CPU请求 | 说明 |
|---------|------|--------|----------|---------|------|
| **PostgreSQL** | postgres-deployment | 1 | 256Mi | 250m | 数据库+PV(10Gi) |
| **Backend** | backend-deployment | 3 | 256Mi | 250m | Django+DRF |
| **Frontend** | frontend-deployment | 2 | 64Mi | 100m | Vue3+Nginx |

### 持久化存储

- **PV**: 10Gi (hostPath: /mnt/data/postgres)
- **PVC**: 5Gi (使用 manual StorageClass)
- **保留策略**: Retain（删除 PVC 后 PV 仍保留数据）

### 网络配置

| 服务 | 类型 | 集群IP | 端口 | NodePort | 说明 |
|------|------|--------|------|----------|------|
| postgres-service | ClusterIP | 自动分配 | 5432 | - | 仅内部访问 |
| backend-service | ClusterIP | 自动分配 | 8000 | - | 仅内部访问 |
| frontend-service | NodePort | 自动分配 | 80 | 30080 | 外部可访问 |

---

## 🔐 安全说明

### 默认密码（生产环境必须修改！）

```bash
# PostgreSQL 密码
用户名: postgres
密码: postgres123 (base64: cG9zdGdyZXMxMjM=)

# Django Secret Key
django-secret-key-change-in-production-environment
```

### 修改密码

```bash
# 1. 生成新密码
NEW_PASSWORD=$(openssl rand -base64 32)
echo -n "$NEW_PASSWORD" | base64

# 2. 编辑 Secret
kubectl edit secret postgres-secret -n login-system
kubectl edit secret backend-secret -n login-system

# 3. 重启服务
kubectl rollout restart deployment/postgres-deployment -n login-system
kubectl rollout restart deployment/backend-deployment -n login-system
```

---

## 📚 相关资源

### 官方文档

- [Kubernetes 官方文档](https://kubernetes.io/)
- [Vue.js 官方文档](https://cn.vuejs.org/)
- [Django 官方文档](https://www.djangoproject.com/)
- [PostgreSQL 官方文档](https://www.postgresql.org/)

### 项目脚本

- `scripts/install-k8s-cn.sh` - K8s 安装脚本（国内镜像）
- `scripts/deploy.sh` - 一键部署脚本
- `scripts/cleanup.sh` - 清理脚本
- `scripts/setup-windows-access.sh` - Windows 访问配置脚本

---

## 💡 最佳实践

### 开发环境

1. 使用 **Port Forward** 方式快速访问
2. 单副本部署节省资源
3. 启用热更新加快开发

### 测试环境

1. 使用 **NodePort** 方式稳定访问
2. 多副本测试高可用
3. 配置域名模拟生产环境

### 生产环境

1. 使用 **Ingress + TLS** 提供 HTTPS
2. 配置资源限制和 HPA
3. 使用外部数据库（如 RDS）
4. 配置监控和日志收集
5. 定期备份数据

---

## 🎉 总结

您现在已经掌握了：

✅ **基础部署**：一键部署到 K8s 集群  
✅ **访问配置**：4种访问方式满足不同场景  
✅ **域名配置**：WSL2 和 Windows 域名访问  
✅ **问题排查**：常见问题的解决方案  
✅ **日常维护**：扩缩容、更新、备份等操作  

**下一步：**

1. 配置 Prometheus + Grafana 监控
2. 实现 CI/CD 自动化部署
3. 添加更多业务功能
4. 进行压力测试和性能优化

---

**🎊 恭喜！您已成功掌握 K8s 登录系统的完整部署！**

**Happy Coding!** 💻

